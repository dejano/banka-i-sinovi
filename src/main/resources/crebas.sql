/*==============================================================*/
/* DBMS tableCode:      Microsoft SQL Server 2008                    */
...
if exists (select 1
   ...
   where r.fkeyid = object_id('FILM') and o.tableCode = 'FK_FILM_ASOCIJACI_ZANR')
alter table FILM
...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('FILM') and o.tableCode = 'FK_FILM_FILMOVI_V_VIDEOTEK')
alter table FILM
...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('FILM') and o.tableCode = 'FK_FILM_ZEMLJA_PO_ZEMLJA_P')
alter table FILM
...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('KOPIJA') and o.tableCode = 'FK_KOPIJA_KOPIJA_FI_FILM')
alter table KOPIJA
...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('UCESNICI_FILMA') and o.tableCode = 'FK_UCESNICI_UCESNICI__FILM')
...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('UCESNICI_FILMA') and o.tableCode = 'FK_UCESNICI_UCESNICI__UCESNIK_')
alter table UCESNICI_FILMA
...
if exists (select 1
           ...
           where  id    = object_id('FILM')
            and   tableCode  = 'FILMOVI_VIDEOTEKE_FK'
            and   indid > 0
            and   indid < 255)
...
if exists (select 1
            from  sysindexes
           where  id    = object_id('FILM')
            and   tableCode  = 'ASOCIJACIJA_FILM_ZANR_FK'
            and   indid > 0
            and   indid < 255)
...
if exists (select 1
            from  sysindexes
           where  id    = object_id('FILM')
            and   tableCode  = 'ZEMLJA_POREKLA_FK'
            and   indid > 0
            and   indid < 255)
...
if exists (select 1
           ...
           where  id    = object_id('UCESNICI_FILMA')
            and   tableCode  = 'UCESNICI_FILMA_FK2'
            and   indid > 0
            and   indid < 255)
...
if exists (select 1
            from  sysindexes
           where  id    = object_id('UCESNICI_FILMA')
            and   tableCode  = 'UCESNICI_FILMA_FK'
            and   indid > 0
            and   indid < 255)
