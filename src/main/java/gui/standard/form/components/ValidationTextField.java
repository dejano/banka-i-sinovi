...
import gui.standard.form.components.document.MaxCharactersDocumentFilter;
import gui.standard.form.components.document.MaxDigitsDocumentFilter;
import gui.standard.form.components.document.StartsWithZeroMaxDigitsDocumentFilter;
import gui.standard.form.components.validation.Patterns;
...
public class ValidationTextField extends JTextField implements IValidationTextField{
	...
	public String getRequitements(){
		...
		if (onlyDigits)
			ret += "Može sadržati samo cifre. ";
		if (!canStartWithZero)
			ret += "Ne sme početi nulom. ";
		if (pattern != null){
			if (patternKey == null)
